...
const pointsArray = [
  ...
  {
    title: "Follow CappyBera on X and turn on notification",
    points: 500,
    content: [
      "To pass this challenge, you must Follow CappyBera on X (Twitter) and turn on your notification.",
      "Please note, we sync this challenge every hour so please be patient and your progress will automatically update.",
    ],
    button: "Follow",
    key: "cappybera_x",
    href: "https://x.com/Cappy_BeraNFTs",
  },
  {
  ...
  {
    title: "Join CappyBera Discord",
    points: 500,
    content: [
      "To pass this challenge, you must Join CappyBera's Discord server and engage with the community.",
      "Please note, we sync this challenge every hour so please be patient and your progress will automatically update. ",
    ],
    button: "Join Discord",
    key: "cappybera_discord",
    href: "https://discord.gg/VvmTrdrFRz",
  },
  {
...
export default function Page() {
  ...
  const checkQuestStatus = useCallback(
    (key: string) => {
      if (!address || !questLists || questLists.length === 0) return false;
  ...
  return (
    <div className="relative space-y-5 overflow-hidden p-5 lg:px-20">
          ...
          <div className="space-y-3 bg-brightBlack p-5 lg:max-w-[450px]">
            ...
            <div className="flex gap-3">
              ...
              <span className="text-gray1">
                Ranked #
                {walletRank && walletRank.rank ? walletRank.rank : "N/A"}
              </span>
            </div>
          </div>
        </div>
                  ...
                  <div className="flex items-center justify-between gap-2 text-xs lg:text-base">
                    <div className="flex items-center">
                      <Checkbox
                        ...
                        isDisabled
                        isSelected={checkQuestStatus(item.key)}
                      />
                      {item.title}
                    </div>
                  ...
                  item.key === "bruuvvprint_pack" ? (
                    <>
                      <Button
                        ...
                        onClick={() => handleMint(item.key, item.href)}
                      >
                        {item.key === "bera_pack"
                          ? renderNFTButtonText(beraPackBalance, "Mint")
                          : item.key === "bruuvvprint_pack"
                            ? renderNFTButtonText(bruvBalance, "Buy")
                            : renderNFTButtonText(smileeberasBalance, "Buy")}
                      </Button>
                    </>
                  ) : (
                    <>
                      <Button
                        ...
                        size="full"
                        onClick={() =>
                          handleClick(item.href, item.key, item.points)
                        }
                        classNames={{
                          base: "text-gray",
                        }}
      ...
      <div className="space-y-3 bg-brightBlack p-5">
        ...
        <p className="text-sm text-gray1">
          You&apos;ve already referred {referredCount} new users, of these{" "}
          {verifiedCount} new users are counted to your referral multiple. After
          your next referral, your new score multiplier will be 2x and your
