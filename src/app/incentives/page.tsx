...
export default function Page() {
  ...
  const token0 = useMemo(
    () =>
      tokenLists.find(
        (token) =>
          token.address.toLowerCase() === selectedPair?.token0.id.toLowerCase(),
      ),
  ...
  const token1 = useMemo(
    () =>
      tokenLists.find(
        (token) =>
          token.address.toLowerCase() === selectedPair?.token1.id.toLowerCase(),
      ),
        ...
        <div className="flex flex-col gap-7 bg-footer p-5 md:p-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                ...
                <ChipBadge>
                  Basic {selectedPair?.stable ? "Stable" : "Volatile"} ·
                  {Number(fee ?? 0) / 100}%
                </ChipBadge>
              </div>
            </div>
          ...
          <div className="flex w-full items-start justify-between gap-3">
              ...
              <div className="flex flex-col items-start justify-start gap-1">
                <div>
                  <span className="text-swapBox">
                    {!!selectedPair ? toSF(selectedPair.reserve0) : 0.0}
                  </span>{" "}
                ...
                <div>
                  <span className="text-swapBox">
                    {!!selectedPair ? toSF(selectedPair.reserve1) : 0.0}
                  </span>{" "}
            ...
            <div className="flex flex-col items-end justify-start gap-2">
              ...
              <div className="flex flex-col items-end justify-start gap-1">
                <div>
                  <span className="text-swapBox">
                    {!!selectedPair ? toSF(token0Deposited) : 0.0}
                  </span>{" "}
                  <span>{selectedPair?.token0.symbol}</span>
                </div>
                ...
                <div>
                  <span className="text-swapBox">
                    {!!selectedPair ? toSF(token1Deposited) : 0.0}
                  </span>{" "}
                  <span>{selectedPair?.token1.symbol}</span>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-2 text-navDefault">
            <div className="flex flex-wrap justify-between">
              <div className="w-[50%] space-y-2 sm:w-fit">
                <p>APR</p>
                <p className="text-white">{toSF(formatEther(rewardRate))}%</p>
              </div>
              <div className="w-[50%] space-y-2 sm:w-fit">
                <p>Current Vote</p>
              ...
              <div className="mt-7 w-full space-y-2 sm:mt-0 sm:w-fit sm:text-right">
                ...
                <p className="text-white">
                  {toSF(availableIncentives)} {selectedToken?.symbol}
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between">
              <p>Your Incentives</p>
              <p>
                Available {tBalance.toFixed(3)} {selectedToken?.symbol}
              </p>
            </div>
          </div>
