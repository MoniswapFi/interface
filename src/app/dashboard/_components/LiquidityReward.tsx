      ...
      <div className="flex flex-col justify-between gap-5 bg-footer p-5 lg:flex-row lg:gap-0">
        ...
        <div className="flex flex-col justify-between gap-5 lg:items-end">
          <div className="lg:text-right">
            ...
            <p>
              {Number(formatUnits(earned, 18)).toLocaleString("en-US", {
        ...
        <div className="flex flex-col justify-between gap-5 lg:items-end">
          <div className="lg:text-right">
            ...
            <p>
              {Number(
                formatUnits(claimable0, Number(data.pair.token0.decimals)),
              ).toLocaleString("en-US", {
                maximumFractionDigits: 5,
                useGrouping: true,
              })}{" "}
              <span className="text-textgray">{data.pair.token0.symbol}</span>
            </p>
            <p>
              {Number(
                formatUnits(claimable1, Number(data.pair.token1.decimals)),
              ).toLocaleString("en-US", {
                maximumFractionDigits: 5,
                useGrouping: true,
              })}{" "}
              <span className="text-textgray">{data.pair.token1.symbol}</span>
            </p>
          </div>
          <button
            ...
            disabled={
              ...
              (claimable0 === BigInt(0) && claimable1 === BigInt(0))
            }
            className="flex cursor-pointer items-center gap-2 text-btn-primary lg:justify-end"
          >
            <Image alt="check icon" src={CheckIcon} />
