...
export const FeeOrIncentiveComponent: FC<FeeOrIncentiveProps> = ({
  ...
  const token = useMemo(
    () =>
      tokenlist.find(
        (tokenType) =>
          tokenType.address.toLowerCase() === tokenAddress.toLowerCase(),
      ),
  ...
  return (
    <div className="flex items-center justify-between gap-3 lg:justify-end">
      <div className="flex items-center gap-3">
        ...
        />
        {Number(formatUnits(earned, token?.decimals ?? 18)).toLocaleString(
          "en-US",
          {
...
export const VotingReward: FC<VotingRewardProps> = ({ data }) => {
  ...
  const token0 = useMemo(
    () =>
      tokenlist.find(
        (token) =>
          token.address.toLowerCase() === data.pair?.token0.id.toLowerCase(),
      ),
  ...
  const token1 = useMemo(
    () =>
      tokenlist.find(
        (token) =>
          token.address.toLowerCase() === data.pair?.token1.id.toLowerCase(),
      ),
  ...
  useEffect(() => {
    (async () => {
      ...
      for (let i = 0; i < length; i++) {
        ...
        const result = resultFromBytes<string>(rewardsAbi, "rewards", resBytes);
        tokens.push(result);
      }
      ...
      <div className="flex flex-col justify-between gap-3 bg-footer p-5 lg:flex-row lg:gap-0">
        <div className="flex items-start">
          ...
          <div>
            ...
            <p className="bg-darkgray px-2 py-1 text-xs text-lightblue">
              Basic {stable ? "Stable" : "Volatile"} ·{Number(fee ?? 0) / 100}%
            </p>
          </div>
        </div>
        ...
        <div className="flex min-w-0 flex-col justify-between gap-5 lg:items-end">
          <div className="flex flex-col gap-3">
            ...
            {Array.from({ length: Number(feesRewardsLength) }, (_, i) => i).map(
              (val) => (
          ...
          {isApprovedToSpendNFT ? (
            ...
            <button
              onClick={() =>
                approveToSpendNFT(__MULTICALL__[chainId], Number(data.lockId))
              }
