...
const TokenSelectableItem: FC<TokenSelectableItemProps> = ({
  ...
  const token0 = useMemo(
    () =>
      tokenlist.find(
        (token) => token.address.toLowerCase() === item.token0.id.toLowerCase(),
      ),
  ...
  const token1 = useMemo(
    () =>
      tokenlist.find(
        (token) => token.address.toLowerCase() === item.token1.id.toLowerCase(),
      ),
    ...
    >
      ...
      <div className="text-right">
        ...
        <p className="text-textlightgray">
          ${toSF((Number(item.reserveUSD) * formattedGaugeTS) / formattedTS)}
        </p>
      </div>
    </button>
  );
    ...
    () =>
      poolsList.filter(
        (pool) =>
          ...
          pool.token0.id.toLowerCase().startsWith(searchValue.toLowerCase()) ||
          pool.token0.symbol
            .toLowerCase()
            .startsWith(searchValue.toLowerCase()) ||
          pool.token1.name
            .toLowerCase()
            .startsWith(searchValue.toLowerCase()) ||
          pool.token1.id.toLowerCase().startsWith(searchValue.toLowerCase()) ||
          pool.token1.symbol
            .toLowerCase()
            .startsWith(searchValue.toLowerCase()),
      ),
            ...
            <ModalBody>
              <div className="flex flex-col gap-5 pb-5">
                <Input
                  ...
                  startContent={
                    <FontAwesomeIcon icon={faSearch} className="text-swapBox" />
                  }
                />
